...
class SessionTest {
    ...
    void create_shouldGenerateLoggedInEvent() {
        ...
        assertTrue(session.pendingEvents().get(0) instanceof com.jguard.domain.auth.event.UserLoggedIn);
    }
    ...
    void logout_shouldGenerateLoggedOutEvent() {
        ...
        // then
        assertEquals(1, session.pendingEvents().size());
        assertTrue(session.pendingEvents().get(0) instanceof com.jguard.domain.auth.event.UserLoggedOut);
    }
}